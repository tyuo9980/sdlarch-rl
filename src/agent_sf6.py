...
def load_act(replan_interval: int = 16):
    print(f"Loading ACT model from {ACT_MODEL_PATH}...")
    checkpoint = th.load(ACT_MODEL_PATH, map_location=DEVICE)
    model      = ACTPolicy(n_classes=checkpoint["n_classes"], n_action_ids=checkpoint["n_action_ids"]).to(DEVICE)
    model.load_state_dict(checkpoint["state_dict"])
    model.input_ints      = checkpoint["input_ints"]
    model.action_id_vocab = checkpoint["action_id_vocab"]
    model.eval()
    print(f"ACT model loaded. ({checkpoint['n_classes']} action classes, {checkpoint['n_action_ids']} action IDs, replan_interval={replan_interval})")
    return ACTInference(model, replan_interval=replan_interval)
...
def main():
    ...
    parser.add_argument("--model", choices=["bc", "lstm", "act"], default="bc",
                        help="Which model to use: 'bc' (MLP), 'lstm', or 'act' (Transformer)")
    ...
    if args.model == "lstm":
        ...
        agent = load_act(replan_interval=args.replan_interval)
    else:
    ...
    with open(STREAM_PATH, "r", errors="ignore") as f:
        ...
        while True:
            ...
            #print(f"infer={t_infer:.2f}ms  gamepad={t_gamepad:.2f}ms")
